...
@section ('title') @parent {{ Lang::get('general.reportes'); }} @stop

@section ('section-title') {{ Lang::get('general.reporte_por') . ' ' . Lang::get('general.' . $type); }} @stop
...
<div class="row">
    ...
    <div class="col-md-12">
        ...
        <a id="exportExcel" href="/report/export/{{ $type }}/{{ $interval }}/excel" class="btn btn-default btn-marginR20"><i class="fa fa-file-excel-o"></i> {{ Lang::get('reports.export_to') }} Excel</a>
        <a id="exportPdf" href="/report/export/{{ $type }}/{{ $interval }}/pdf" class="btn btn-default btn-marginR20"><i class="fa fa-file-pdf-o"></i> {{ Lang::get('reports.export_to') }} PDF</a>
    </div>
</div>
...
<script>
    $(document).ready(function(){
        ...
        $('#tbl_reportes').html(loader).load('/report_table/{{ $type }}/{{ $interval }}');        
        ...
        function(start, end) {        
            ...
            var url = '/report_table/{{ $type }}/' + start.format('YYYY-M-D') + '-to-' +  end.format('YYYY-M-D');
            $('#tbl_reportes').html(loader).load(url);
            
            $('#exportExcel').attr('href', '/report/export/{{ $type }}/' + start.format('YYYY-M-D') + '-to-' +  end.format('YYYY-M-D') + '/excel');
            $('#exportPdf').attr('href', '/report/export/{{ $type }}/' + start.format('YYYY-M-D') + '-to-' +  end.format('YYYY-M-D') + '/pdf');
        });
    });
</script>
