 ...
 * @see https://github.com/adoy/PHP-OAuth2
...
/**
     ...
     * @return array
     */
    public function fetch($protected_resource_url, array $parameters = array(), $http_method = self::HTTP_METHOD_GET, array $http_headers = array(), $encode=false,  $form_content_type = self::HTTP_FORM_CONTENT_TYPE_APPLICATION)
    {
        ...
        return $this->executeRequest($protected_resource_url, $parameters, $http_method, $http_headers, $encode, $form_content_type);
    }
     ...
     * @return array
     */
    private function executeRequest($url, array $parameters = array(), $http_method = self::HTTP_METHOD_GET, array $http_headers = null, $encode=true, $form_content_type=self::HTTP_FORM_CONTENT_TYPE_MULTIPART)
    {        
        ...
        if ($curl_error = curl_error($ch)) {
            ...
            $json_decode = $encode ? json_decode($result) : $result;
        }
        curl_close($ch);

        return array(
            'body' => (null === $json_decode) ? $result : $json_decode,
            'code' => $http_code,
